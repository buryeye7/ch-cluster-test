...
check_sync() {
    ...
    while true
    do
        seed_height=$(kubectl exec $gaia_seed --container gaia-seed -- gaiacli query block --chain-id=testnet | jq .block.header.height | sed "s/\"//g")
        node3_height=$(kubectl exec $gaia_node3 --container gaia-node3 -- gaiacli query block --chain-id=testnet | jq .block.header.height | sed "s/\"//g")
...
if false;then
    ...
    do
        ...
        expect -c "
        set timeout 3
        spawn kubectl exec $GAIA_SEED -it --container gaia-seed -- gaiacli tx staking create-validator --amount=1000000uatom --pubkey=$node_pubkey --moniker=solution$i --chain-id=testnet --commission-rate="0.10" --commission-max-rate="0.20" --commission-max-change-rate="0.01" --min-self-delegation="1" --gas="auto" --gas-prices="0.025uatom" --from $wallet_alias   
        expect "passphrase:"
            send \"$PW\\r\"
        expect eof
        "
